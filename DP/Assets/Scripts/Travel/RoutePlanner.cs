...
{
    /// <summary>
    /// Фабрика для создания ECS-сущностей караванов.
    /// Проверяет выбранные города и формирует Blob-ассеты маршрута и груза.
    /// </summary>
    public class RoutePlanner : IRoutePlanner
    {
        ...
        public Entity CreateConvoy(int[] cityIndices, CargoItem[] cargoItems)
        {
            if (cityIndices == null || cityIndices.Length < 2 || cityIndices.Length > 5)
                throw new ArgumentException("Маршрут должен содержать от 2 до 5 городов.");

            foreach (int cityIdx in cityIndices)
                _ = _worldState.GetCity(cityIdx); // проверка существования города

            // Создание сущности со всеми компонентами
            ...
            _entityManager.SetComponentData(entity, new ConvoyStateComponent { State = ConvoyState.Idle, Progress = 0f });
            _entityManager.SetComponentData(entity, new EventTimerComponent { TimeUntilCheck = 60f });

            // Построение Blob-ассета маршрута
